    ...
    *,
    ...
    while True:
        if time.monotonic() - started > timeout_s:
            break
        items = await poll_children(base_url, kb_id, list(token_to_info.keys()))
        last_items = items
        now_ms = _now_ms()
        for it in items:
            ...
            if status in ("indexed", "error") and rid not in terminal_at:
                # Prefer server-provided updated_at if present
                terminal_at[rid] = int(it.get("updated_at", now_ms))
        if len(terminal_at) == len(uploaded):
            break
    ...
    if not all_terminal or success == 0:
        return 1
    return 0
